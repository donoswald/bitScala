...
class OpValueSpec extends UnitSpec {
  ...
  it should "checksig" in {
    ...
    assert(OpValue.OP_CHECKSIG.execute(stack, z) == true)
    assert(stack.size() == 1)
  ...
  it should "checksig verify" in {
    ...
    assert(OpValue.OP_CHECKSIGVERIFY.execute(stack, z) == true)
  ...
  it should "multisig" in {
    ...
    assert(OpValue.OP_CHECKMULTISIG.execute(stack, z) == true)
    assert(stack.size() == 1)
