...
class S256PointSpec extends UnitSpec {
  ...
  it should "serialize in sec format" in {
    var coefficient = BigInt(999).pow(3)
  ...
  it should "test secrets and public points" in {
    ...
    points(0)(0) = 7
    points(0)(1) = BigInt("5cbdf0646e5db4eaa398f365f2ea7a0e3d419b7e0330e39ce92bddedcac4f9bc", 16)
    points(0)(2) = BigInt("6aebca40ba255960a3178d6d861a54dba813d0b813fde7b5a5082628087264da", 16)
    points(1)(0) = 1485
    points(1)(1) = BigInt("c982196a7466fbbbb0e27a940b6af926c1a74d5ad07128c82824a11b5398afda", 16)
    points(1)(2) = BigInt("7a91f9eae64438afb9ce6448a1c133db2d8fb9254e4546b6f001637d50901f55", 16)
    points(2)(0) = BigInt(2).pow(128)
    points(2)(1) = BigInt("8f68b9d2f63b5f339239c1ad981f162ee88c5678723ea3351b7b444c9ec4c0da", 16)
    points(2)(2) = BigInt("662a9f2dba063986de1d90c2b6be215dbbea2cfe95510bfdf23cbf79501fff82", 16)
    points(3)(0) = BigInt(2).pow(240) + BigInt(2).pow(31)
    points(3)(1) = BigInt("9577ff57c8234558f293df502ca4f09cbc65a6572c842b39b366f21717945116", 16)
    points(3)(2) = BigInt("10b49c67fa9365ad7b90dab070be339a1daf9052373ec30ffae4f72d5e66d053", 16)
  ...
  it should "generate address" in {
    ...
    var arr = Array(
      (BigInt(888).pow(3), "148dY81A9BmdpMhvYEVznrM45kWN32vSCN", "mieaqB68xDCtbUBYFoUNcmZNwk74xcBfTP", true),
      (BigInt(321), "1S6g2xBJSED7Qr9CYZib5f4PYVhHZiVfj", "mfx3y63A7TfTtXKkv7Y6QzsPFY6QCBCXiP", false),
      (BigInt("4242424242"), "1226JSptcStqn4Yq9aAmNXdwdc2ixuH9nb", "mgY3bVusRUL6ZB2Ss999CSrGVbdRwVpM8s", false))
